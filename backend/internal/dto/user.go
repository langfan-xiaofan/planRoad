...
type UserPictureRes struct {
	...
	School          string   `bson:"school"`           //学历
	Job             string   `bson:"job"`              //推荐的工作
	Position        string   `bson:"position"`         //推荐的职位
	Skills          []string `bson:"skills"`           //会的技能
	Certifications  []string `bson:"certifications"`   //证书
	Innovation      string   `bson:"innovation"`       //创新能力
	LearningAbility string   `bson:"learning_ability"` //学习能力
	StressTolerance string   `bson:"stress_tolerance"` //抗压能力
	Communication   string   `bson:"communication"`    //沟通能力
	Experience      string   `bson:"experience"`       //项目经验
	PublicBase      []string `bson:"publicbase"`       //基础
}
